...
export default function NovaReclamacao() {
  ...
  function update(field, value) {
    if (field === "anexos") {
      ...
      const nextFiles = toArray(value).filter(isFileObject);
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (missingGasEndpoint) {
        setResultMsg(
          "Configuração ausente: defina VITE_APPSCRIPT_URL (URL do Apps Script /exec)."
        );
        return;
      }
      ...
      <div className="max-w-2xl mx-auto py-4 md:py-8">
        <div className="text-center mb-6 md:mb-8 space-y-3 md:space-y-4">
          ...
          <p className="text-sm md:text-base text-slate-600 px-3">
            Registre sua reclamação sobre o transporte coletivo. Sua opinião é
            importante para melhorarmos nossos serviços.
          </p>
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mx-2 md:mx-0">
            <p className="text-green-700 text-xs md:text-sm">
              Preencha todas as etapas e guarde seu protocolo.
            </p>
          </div>
        </div>
        ...
        {missingGasEndpoint && (
          <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Configure <code className="font-mono">VITE_APPSCRIPT_URL</code> com a URL <code>/exec</code> publicada do Apps Script.
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow mb-6">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            {step === 2 && (
              <StepDescricao form={form} update={update} errors={errors} />
            )}
            {step === 3 && (
              <StepContato form={form} update={update} errors={errors} />
            )}
            {step === 4 && (
              <Success
...
function StepDados({ form, update, errors, ASSUNTOS, LINHAS }) {
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      <Field label="Tipo de ônibus (opcional)">
        <input
    ...
    <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div className="md:col-span-2">
        <Field
          ...
          hint="Mínimo 20 caracteres. Evite dados pessoais."
        >
          <textarea
    ...
    <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      <div className="md:col-span-2">
        <label className="flex items-start gap-3 p-4 rounded-lg border bg-gray-50">
          ...
          <span className="text-sm text-gray-700">
            Desejo ser contatado(a) sobre esta reclamação.
          </span>
        </label>
      </div>
      ...
      <div className="md:col-span-2">
        ...
        >
          ...
          />
          <span className="text-sm text-gray-700">
            Li e concordo com o tratamento dos meus dados pessoais nos termos da
            LGPD para fins de registro e resposta desta reclamação.
          </span>
        </label>
        {errors.lgpd_aceite && (
